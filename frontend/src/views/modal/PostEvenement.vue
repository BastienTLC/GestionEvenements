  ...
  try {
      const response = await EvenementService.createEvenement(postInput.value);
      console.log(response.data); // Affiche les données récupérées depuis l'API
    } catch (error) {
      console.error('Error loading evenements:', error);
    }
  ...
  const createEvenement = () => {
    ...
    if (postInput.value.nom === "" || postInput.value.date === "" || postInput.value.duree === "" || postInput.value.nombreMaxPersonne === 0 || lieu.value === null) {
      ...
      const dateISO = new Date(postInput.value.date).toISOString();

      // Convertir la durée en format HH:mm:ss
      const dureeTime = new Date(postInput.value.duree).toTimeString().split(' ')[0];

      // Mettre à jour les valeurs de postInput avec les valeurs converties
      postInput.value.date = dateISO;
      postInput.value.duree = dureeTime;
        ...
        <StepperPanel>
          ...
          <template #content="{ prevCallback, nextCallback }">
            ...
            <div class="flex pt-4 justify-content-between">
              ...
              <Button label="Next" icon="pi pi-arrow-right" iconPos="right" @click="() => { if (createEvenement()) { nextCallback(); } else { prevCallback(); } }" />
            </div>
          </template>
        </StepperPanel>
        <StepperPanel>
